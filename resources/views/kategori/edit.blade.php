...
<div class="max-w-2xl mx-auto space-y-8 animate-in slide-in-from-bottom-8 duration-700">
    ...
    <div class="flex items-center gap-4">
        ...
        <div>
            ...
            <p class="text-gray-500 mt-1 font-medium">Perbarui informasi kategori dan kode prefix.</p>
        </div>
    </div>
    ...
    <div class="bg-white rounded-3xl shadow-xl shadow-gray-200/50 border border-gray-100 overflow-hidden transform transition-all">
        <form action="{{ route('admin.categories.update', $category) }}" method="POST" class="p-8 space-y-8 text-left">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-6">
                ...
                <!-- Kode Kategori -->
                <div class="space-y-2">
                    <label for="kode_kategori" class="text-sm font-bold text-gray-700 ml-1">Kode Prefix</label>
                    <div class="relative group">
                        <div class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none text-gray-400 group-focus-within:text-indigo-500 transition-colors">
                            <i data-lucide="hash" class="w-5 h-5"></i>
                        </div>
                        <input type="text" name="kode_kategori" id="kode_kategori" value="{{ old('kode_kategori', $category->kode_kategori) }}" required maxlength="5"
                    ...
                    <p class="text-xs text-gray-400 font-medium ml-1 text-rose-400">Peringatan: Mengubah kode prefix akan mempengaruhi penomoran alat baru kedepannya.</p>
                    @error('kode_kategori')
                        <p class="text-rose-500 text-xs font-bold mt-1 ml-1">{{ $message }}</p>
                    @enderror
                </div>
